...
__global__ void matrixMultiplyShared(const float *__restrict__ A, 
  ...
  for (int k = 0; k < (numAColumns + TILE_SIZE - 1) / TILE_SIZE; ++k) {
    const unsigned int idAx = k * TILE_SIZE + threadIdx.x;
    float aValue = 0.f;
    if (idy < numARows && idAx < numAColumns) {
      aValue = A[idy * numAColumns + idAx];
    }
    tileA[threadIdx.y][threadIdx.x] = aValue;

    const unsigned int idBy = k * TILE_SIZE + threadIdx.y;
    float bValue = 0.f;
    if (idBy < numBRows && idx < numBColumns) {
      bValue = B[idBy * numBColumns + idx];
    }
    tileB[threadIdx.y][threadIdx.x] = bValue;
    __syncthreads();
    
    for (int p = 0; p < TILE_SIZE; ++p) {
      ans += tileA[threadIdx.y][p] * tileB[p][threadIdx.x];
    } 
    __syncthreads();
  }
...
int main(int argc, char **argv) {
  ...
  gpuTKLog(TRACE, "The dimensions of tiles are ", TILE_SIZE, " x ", TILE_SIZE);
