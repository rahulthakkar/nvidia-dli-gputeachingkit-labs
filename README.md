# Accelerated Computing Teaching Kit Lab Repository

Welcome to the Accelerated Computing Teaching Kit Lab repository. The kit and associated labs are produced jointly by NVIDIA and University of Illinois (UIUC).  All material is available under the [Creative Commons Attribution-NonCommercial License](http://creativecommons.org/licenses/by-nc/4.0/).
...
Learn how to run these labs on AWS GPUs in the cloud using the [NVIDIA-Docker instructions](#markdown-header-using-docker).
...
Please follow the instructions [here](https://bitbucket.org/nvidia-dli/gputeachingkit-labs/src/master/Module21/OpenACCVectorAdd/PGI_LICENCE_INFO.markdown?fileviewer=file-view-default) in this Bitbucket repository to download the
...
~~~{.bash}
git clone git@bitbucket.org:nvidia-dli/gputeachingkit-labs.git
~~~

**Observation to users with an existing git clone obtained previous to 07/15/2020, we strongly recommend getting a fresh clone instead of performing a git pull for obtaining the updated repo.**
...
* `BUILD_LIBgpuTK_LIBRARY` -- option toggles whether to build the `libgpuTK` (the support library)
...
* `BUILD_JSONLOG` -- option toggles whether to build the `libgpuTK` with detailed
  json logging to stdout. This option is meant for extreme debugging of the code logs.
* `BUILD_LOGTIME` -- option toggles whether to build the `libgpuTK` with timers printing
   to stdout the elapsed time in `ms` for the operation.

Templates are meant to be used as starting code for students.
...
The NVIDIA Accelerated Computing Teaching Kit includes links to free online DLI courses – **a value of up to $90 per person per course**. DLI training reinforces deep learning concepts presented in the Teaching Kits and teaches students how to apply those concepts to end-to-end projects. Through built-in assessments, students can earn certifications that prove subject matter competency and can be leveraged for professional career growth. Each course presents a self-paced learning environment with access to a GPU-accelerated workstation in the cloud. All students need is a web browser and Internet connection to get started. **Although these courses are designed to be taken online, you are free to administer them to your students in a live setting as well.**

The recommended DLI course (with certificate opportunity) for students learning through the Accelerated Computing Teaching Kit is **[Getting Started with Accelerated Computing in Modern CUDA C++](https://learn.nvidia.com/courses/course-detail?course_id=course-v1:DLI+S-AC-04+V2)**.

The Accelerated Computing Teaching Kit [syllabus](https://developer.nvidia.com/accelerated-computing-teaching-kit-syllabus) suggests students take sections of this full-day course throughout the full syllabus. It also suggests shorter courses that can be used as labs throughout your university course.

*To enable these or any other courses for your students, please create a case [here](https://enterprise-support.nvidia.com/s/create-case) using your NVIDIA Developer email address with subject line “AC Teaching Kit Online Course Access”. You will then receive information about how to give free access to your students.*
...
#### Become a DLI Certified Instructor
Join the University Ambassador Program to teach DLI courses at your university to students, faculty, and researchers at no cost. Educators can learn more and apply [here](https://www.nvidia.com/en-us/training/educator-programs/university-ambassador-program/).
